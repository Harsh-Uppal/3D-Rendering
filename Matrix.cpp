...
Matrix::Matrix(std::vector<float> data)
{
	...
	this->_mat = std::vector<std::vector<float>>(data.size());

	for (size_t i = 0; i < data.size(); i++)
		_mat.push_back(std::vector<float>{data[i]});
...
Matrix Matrix::Multiply(Matrix mat1, Matrix mat2) {
	...
	Matrix rotatedMat2 = Matrix::Rotate(mat2, RotationManner::AntiClockWise);

	if (mat1.GetColumns() != rotatedMat2.GetColumns()) {
		throw new std::exception("Mat1's columns must equal Mat2's rows");
		return Matrix();
	}

	Matrix newMat = Matrix(rotatedMat2.GetRows(), mat1.GetRows());
	...
	for (int r = 0; r < newMat.GetRows(); r++)
		for (int c = 0; c < newMat.GetColumns(); c++)
		{
			...
			for (int cOfMat1 = 0; cOfMat1 < mat1.GetColumns(); cOfMat1++)
				...
				rotatedMat2.GetValue(cOfMat1, rotatedMat2.GetRows() - c - 1);
...
Matrix Matrix::Rotate(Matrix mat, RotationManner manner) {
	...
	if (manner == RotationManner::AntiClockWise)
	...
	else if (manner == RotationManner::ClockWise)
		for (int row = 0; row < mat.GetRows(); row++)
			for (int column = 0; column < mat.GetColumns(); column++)
...
Matrix Matrix::RotateClockWise(Matrix mat) {
	return Matrix::Rotate(mat, Matrix::RotationManner::ClockWise);
}
Matrix Matrix::RotateAntiClockWise(Matrix mat) {
	return Matrix::Rotate(mat, Matrix::RotationManner::AntiClockWise);
}